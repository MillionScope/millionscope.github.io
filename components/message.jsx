...
const PurePreviewMessage = ({ chatId, message, vote, isLoading, setMessages, reload, isReadonly }) => {
  ...
  console.log("PurePreviewMessage.message", JSON.stringify(message))
  // console.log("toolInvocations", message.toolInvocations)

  return (
    <AnimatePresence>
      <motion.div className="w-full mx-auto max-w-3xl px-4 group/message" initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1 }} data-role={message.role}>
        <div
...
export const ThinkingMessage = () => {
  ...
  return (
    <motion.div className="w-full mx-auto max-w-3xl px-4 group/message " initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { delay: 1 } }} data-role={role}>
      <div
        className={cx(
